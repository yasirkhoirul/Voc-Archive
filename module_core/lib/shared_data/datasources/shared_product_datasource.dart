...
class SharedProductDatasourceImpl implements SharedProductDatasource {
  ...
  Future<List<ProductModel>> getAllProducts({
    ...
    return await (() async {
      ...
      Logger().d("Fetching products with filters - query: $query, types: $types, minPrice: $minPrice, maxPrice: $maxPrice");
      if (types != null && types.isNotEmpty) {
        queryRef = queryRef.where('type', whereIn: types);
      }
      
      final snapshot = await queryRef.get();
      var products = snapshot.docs.map((doc) => ProductModel.fromJson(doc.data() as Map<String, dynamic>)).toList();
      
      if (query != null && query.isNotEmpty) {
        final search = query.toLowerCase();
        products = products.where((p) => p.namaBrand.toLowerCase().contains(search)).toList();
      }
  ...
  Future<List<ProductModel>> getDiscountProducts({String? query}) async {
    return await (() async {
      ...
      var products = snapshot.docs.map((doc) => ProductModel.fromJson(doc.data())).toList();
      if (query != null && query.isNotEmpty) {
        final search = query.toLowerCase();
        products = products.where((p) => p.namaBrand.toLowerCase().contains(search)).toList();
      }
