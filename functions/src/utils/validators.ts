...
/**
 * Validates that gambar is a non-empty array of strings.
 */
export function validateGambar(gambar: unknown): void {
    if (!Array.isArray(gambar) || gambar.length === 0) {
        throw new Error(`"gambar" must be a non-empty array of image URLs.`);
    }
    for (const url of gambar) {
        if (typeof url !== "string" || url.trim().length === 0) {
            throw new Error(`Each item in "gambar" must be a non-empty string URL.`);
        }
    }
}
